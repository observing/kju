 ...
 *  - `ms` Amount of milliseconds to increase or decrease the interval. Defaults
 *    to 100 ms
...
function kju (configuration) {
  ...
  fs.exists(this.path, function exists (exists) {
    if (!exists) {
...
/**
 * Queues up the items until it reaches the limit.
 *
 * @returns {kju}
 * @api public
 */
...
kju.prototype.recovery = function recover () {
   ...
   *
   * @param {Error} err Set when a error occurred
